...
export const TOOLS: ToolDef[] = [
  ...
  { name: 'get_workspace', description: 'One workspace in full: entry, file tree, images and fonts with their framing, asset folders, and the figure slots in main.typ (index, line, caption, placed path, height in points).', schema: { workspace_id: WS }, run: async (a, d) => { const det = await d.service.detail(a.workspace_id as string); return { ...det, slots: slotsOf(readSource(d, det.entry.id, 'main.typ')) }; } },
