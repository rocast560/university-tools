...
export function openapiDocument() {
  return {
    ...
    paths: {
      ...
      '/api/projects/{id}/edit/add': { post: op('Add a component to the schematic (backed up first)', { requestBody: body({ type: 'object', properties: { part: { type: 'string', description: 'Alias like "resistor" or "LED", or a lib_id like "Device:R"' }, libId: { type: 'string', description: 'KiCad lib_id, e.g. "Device:R" (alternative to part)' }, value: { type: 'string' }, ref: { type: 'string', description: 'Reference to use; default: next free' }, connections: { type: 'object', additionalProperties: { type: 'string' }, description: 'pin number -> net name' } } }) }) },
      '/api/projects/{id}/edit/connect': { post: op('Join a pin to a net (label or power symbol)', { requestBody: body({ type: 'object', required: ['ref', 'pin', 'net'], properties: { ref: { type: 'string' }, pin: { type: 'string' }, net: { type: 'string' } } }) }) },
      '/api/projects/{id}/edit/disconnect': { post: op('Remove the labels this tool placed on a pin', { requestBody: body({ type: 'object', required: ['ref', 'pin'], properties: { ref: { type: 'string' }, pin: { type: 'string' } } }) }) },
      '/api/projects/{id}/edit/remove': { post: op('Remove a component and the labels placed on it', { requestBody: body({ type: 'object', required: ['ref'], properties: { ref: { type: 'string' } } }) }) },
      '/api/projects/{id}/edit/value': { post: op('Set a component\'s Value field', { requestBody: body({ type: 'object', required: ['ref', 'value'], properties: { ref: { type: 'string' }, value: { type: 'string' } } }) }) },
